...
export function SupportChatWidget() {
  ...
  useEffect(() => {
    ...
    if (stored) {
      ...
      const newId = `visitor_${Date.now()}_${Math.random().toString(36).slice(2, 11)}`
    ...
    if (storedConv) {
      try {
        const data = JSON.parse(storedConv) as {
  ...
  useEffect(() => {
    scrollToBottom()
  }, [messages, isLoading, isOpen, showIntro, scrollToBottom])
  ...
  const sendMessage = async () => {
    ...
    try {
      ...
      const data = (await res.json()) as { error?: string; message?: string; conversationId?: string }
      ...
      setMessages((prev) => [
        ...
        {
          role: "assistant",
          content: `Something went wrong (${detail}). Please try again or email hello@vobizsuite.io`,
        },
      ])
    } finally {
      setIsLoading(false)
  ...
  const requestEscalation = async () => {
    ...
    try {
      ...
      const data = (await res.json()) as { escalated?: boolean; message?: string }

      if (data.escalated) {
        setIsEscalated(true)
        const escalationText = typeof data.message === "string" ? data.message : ""
        if (escalationText) {
          setMessages((prev) => [...prev, { role: "assistant", content: escalationText }])
        }
      }
    } catch (err) {
  ...
  const endSessionAndMaybeEmail = useCallback(
    async (opts: { clearStorage: boolean }) => {
      ...
      if (email && messages.length > 1) {
        try {
          ...
          // keepalive may still deliver; avoid noisy errors
        }
      }
    ...
    [visitorEmail, visitorName, messages]
  )

  const handleClosePanel = () => {
    void endSessionAndMaybeEmail({ clearStorage: false })
    setIsOpen(false)
  }

  const clearConversation = () => {
    void endSessionAndMaybeEmail({ clearStorage: true })
  }
          ...
          {showIntro ? (
            <div className="flex flex-1 flex-col items-center justify-center gap-4 overflow-y-auto p-6">
              ...
              <div className="w-full space-y-3">
                ...
                <Input
                  type="email"
                  placeholder="Email (for transcript when you close chat)"
                  value={visitorEmail}
                  onChange={(e) => setVisitorEmail(e.target.value)}
                  className="bg-background"
                />
                <Button className="w-full" onClick={startConversation} disabled={!visitorName.trim()}>
                  Start Conversation
                </Button>
              </div>
            </div>
